...
class YoloAnnotationCodec:
    """Port adapter for application services that store immutable YOLO labels."""

    def encode(self, boxes: Iterable[AnnotationBox]) -> bytes:
        return serialize_yolo(boxes).encode("utf-8")

    def decode(self, content: bytes) -> tuple[AnnotationBox, ...]:
        return parse_yolo_text(content.decode("utf-8-sig"), source=Path("<artifact>"))
